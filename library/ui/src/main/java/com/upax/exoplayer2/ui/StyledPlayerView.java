...
import com.upax.exoplayer2.ui.AspectRatioFrameLayout.ResizeMode;
  ...
  public StyledPlayerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    if (attrs != null) {
      ...
      try {
        ...
        showBuffering = a.getInteger(R.styleable.StyledPlayerView_show_buffering_change, showBuffering);
      ...
      switch (surfaceType) {
        ...
        case SURFACE_TYPE_SPHERICAL_GL_SURFACE_VIEW:
          try {
            Class<?> clazz =
                Class.forName(
                    "com.upax.exoplayer2.video.spherical.SphericalGLSurfaceView");
        ...
        case SURFACE_TYPE_VIDEO_DECODER_GL_SURFACE_VIEW:
          try {
            Class<?> clazz =
                Class.forName("com.upax.exoplayer2.video.VideoDecoderGLSurfaceView");
            surfaceView = (View) clazz.getConstructor(Context.class).newInstance(context);
          } catch (Exception e) {
            throw new IllegalStateException(
